 ...
 */
import { BOARD_SIZE, THEME_COLORS, isPlayableSquare, positionsEqual, } from '../types.js';
/**
 * Default render configuration
 */
function createDefaultConfig(canvasSize) {
...
export class Renderer {
     ...
     */
    animateMove(piece, from, to, duration = 300) {
        return new Promise(resolve => {
     ...
     */
    async animateMultiJump(piece, path, durationPerHop = 250) {
        if (path.length < 2) {
            return;
        }
    ...
    _drawGhostPieces() {
        ...
        for (const ghost of this._ghostPieces) {
            ...
            const color = ghost.player === 'human' ? colors.pieceHuman : colors.pieceAgatha;
    ...
    _drawPieces() {
        ...
        if (this._animatingPiece && this._animationFrom && this._animationTo) {
            const fromX = this._animationFrom.col * cellSize + cellSize / 2;
            const fromY = this._animationFrom.row * cellSize + cellSize / 2;
     ...
     */
    _drawPiece(piece, x, y) {
        const { pieceRadius, colors } = this._config;
        const color = piece.player === 'human' ? colors.pieceHuman : colors.pieceAgatha;
        // Check if this piece is glowing
        const isGlowing = this._glowingPiece &&
        ...
        if (isGlowing) {
            ...
            const gradient = this._ctx.createRadialGradient(x, y, pieceRadius * 0.5, x, y, pieceRadius * 2);
        ...
        this._ctx.fill();
        // Draw piece body
        const gradient = this._ctx.createRadialGradient(x - pieceRadius * 0.3, y - pieceRadius * 0.3, 0, x, y, pieceRadius);
     ...
     */
    getPositionFromCoords(screenX, screenY) {
        const rect = this._canvas.getBoundingClientRect();
